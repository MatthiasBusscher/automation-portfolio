...
- **Registered area as the source of truth, knowing its limits.** BAG `oppervlakte` is the legally registered usable area — authoritative and consistent, but not always identical to the *serviceable* area (see limitations). Consistency across all quotes was worth more than per-building precision arguments.
- **Write-back to a dedicated CRM field.** The m² lands where the quote calculation already reads, so sales can price a deal seconds after entering an address without leaving the CRM.

## The hardest part

Address string sanitization. The pipeline is only as good as its match rate, and the raw failure cases were all input formatting: "Straatweg 12, 1234 AB Amsterdam, Nederland" vs "straatweg 12 amsterdam". Building the nested replace/normalization logic that pushed messy real-world strings to a reliable PDOK match — without mangling legitimate address parts like "1e Constantijn Huygensstraat" — was iterative, driven by actual failed lookups. In practice, every address attempted resolved successfully once the normalization was in place.

## Results

- Sales generates a correctly priced quote immediately after entering an address — no register lookups, no leaving the CRM.
...
- Generic version of this chain (address → BAG data) is on the open-source list — it's useful to any Dutch company that quotes on buildings.
