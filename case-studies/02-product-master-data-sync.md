...
Every new or changed product had to be entered manually in three places: the central product database, the CRM (Pipedrive), and the invoicing system (WeFact). With no single source of truth, teams worked from outdated catalogs and invoices could carry inconsistent product data. Product changes happened periodically and when supplier catalog updates were communicated. The administration structure added real complexity: some product types needed to exist in multiple administrations at once, while others belonged to a single administration. Keeping that straight by hand was unrealistic.
...
- Invoicing errors from stale product names, outdated product data, or wrong entities eliminated — invoices always reference the synced catalog.
