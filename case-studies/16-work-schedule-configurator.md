...
> **Context** Operational calculation & sales · composing facility work schedules (object → rooms → elements → actions)
...
A facility contract's work schedule is a large matrix: which object type, which room types, which elements (floors, desks, windows) in those rooms, and which actions per element (vacuum, mop). Calculators assembled these by copying from large source documents — slow, and structurally error-prone in a specific way: nothing prevented *illogical combinations* ("vacuum" as an action on a window) from reaching client documents. A bespoke web application would solve it but was too costly and slow to build; the constraint was to deliver configurator-grade UX inside Google Sheets without the browser grinding to a halt.
...
- **Undo via `PropertiesService`.** `undoLastPush` stores the last export's target coordinates, so a wrong push into a *client* template is reversible with one click instead of a manual cleanup in a delivery document. Small feature, outsized trust effect: people use a push button much more freely when there's an undo.
...
- Composing a complex schedule (500+ rows) went from hours of copy-paste to minutes of clicking plus a one-click export.
- The interface responds without perceptible load times — uncommon for script-driven spreadsheets and decisive for adoption.
- Faulty exports are recoverable in one click without damaging client files.
...
- The configurator handles composition only — which elements and actions apply to a room type. Quantities, frequencies, and pricing were out of scope and handled in a separate step.
