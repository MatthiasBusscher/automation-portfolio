...
A won deal triggers the real work: customer data, product line-items, frequencies, and signed contracts must travel from the CRM into the operational planning systems. Done manually, this handover corrupted data exactly where it hurt — missing financial codes, mistyped product quantities, work orders lost in inboxes. Three things made naive automation insufficient: sales marked deals "won" before they were financially complete (no debtor number yet); deals carry *nested* multi-product data that a flat spreadsheet row can't hold; and one-time work orders required the signed contract *file* to move along with the data.
...
flowchart LR
    ...
    ROUTE -- "Work Order" --> FILE["Download signed contract<br/>→ archive in Shared Drive"]
    FILE --> EXTRACT
    EXTRACT --> TRANSFORM["Iterate + aggregate:<br/>clean JSON object<br/>(SKU, qty, discounts, VAT)"]
    TRANSFORM --> LOAD[("Load into _ImportData<br/>tab of ops backend")]
```

A strict gatekeeper webhook fires only when a deal is won *and* carries both financial integration IDs — i.e., only after the upstream [debtor](06-order-to-cash-automation.md) and [cost-center](07-multi-entity-cost-centers.md) flows have done their work. The flow then routes by contract type, extracts and transforms nested product data into a clean JSON object, archives signed work-order contracts to the operational Drive, and loads everything into the planning backend's staging tab.
...
- **JSON-in-a-cell as the transport format for line-items.** A deal's products are a variable-length nested structure; flattening to columns either truncates or explodes the schema. Aggregating line-items into one clean JSON string (SKU, quantities, discounts, VAT) keeps the staging interface stable while the GAS backend parses it into whatever structure planning needs. Unorthodox, deliberate, and it worked — the schema never had to change for product-count reasons.
- **Files travel with the data.** For work orders, the signed contract is downloaded from the CRM and archived in the correct Shared Drive in the same run — the handover is atomic from operations' perspective: data *and* paperwork arrive together or not at all.
...
The transform step. Pipedrive returns deal products as nested API resources that must be fetched separately, iterated, enriched (SKU, discount, VAT per line), and aggregated into one syntactically valid JSON string using Make's iterator/aggregator semantics — where escaping, separators, and empty-product edge cases all conspire against you. Producing JSON that *always* parses on the GAS side, for every deal shape sales could produce, took most of the build time.

## Results

- Operations never receives an incomplete customer dossier — completeness is enforced before transmission, not checked after.
- Multi-line product structures arrive as clean, parseable data; mistyped quantities and lost financial codes are gone from the handover.
- Signed work orders are archived in the right operational location automatically, in the same transaction as the data.
