...
- **Push-based, like every CRM flow in this portfolio.** The KvK API and Make are only hit when a number is actually added or changed — relevant here because the KvK API is metered (€6.20/month base subscription plus a few cents per call), and the push-only trigger keeps usage strictly proportional to actual data changes.
